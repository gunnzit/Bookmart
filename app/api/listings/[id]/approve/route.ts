...
export async function GET(request: Request, context: any) {
  try {
    ...
    // Fire WhatsApp notification (non-blocking — won't break approval if it fails)
    if (listing.seller?.phone) {
      const message = action === 'approve'
        ? `✅ *Your listing is live on BuddyBooks!*\n\n📚 *${listing.title}*\n💰 ₹${listing.price}\n\nView it here: https://buddybooks.in/listing/${listing.id}\n\n_Bedi Book Store · BuddyBooks_`
        : `❌ *Your listing was not approved.*\n\n📚 *${listing.title}*\n\nReason: Does not meet our listing guidelines. Please check your photos (MRP tag must be visible) and re-list.\n\nQuestions? Chat with us: https://wa.me/919914735738\n\n_Bedi Book Store · BuddyBooks_`

      sendWhatsApp(listing.seller.phone, message).catch(() => {
        // Silently ignore — approval is unaffected
      })
    }
