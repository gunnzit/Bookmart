    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(27,42,74,0.10) !important; }
        ...
        .skeleton { background: linear-gradient(90deg, #f0ede8 25%, #e8e4de 50%, #f0ede8 75%); background-size: 800px 100%; animation: shimmer 1.4s infinite; }
      `}</style>

      <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>

        {/* Nav */}
        <nav style={{ background: '#fff', borderBottom: '1.5px solid #EDE9E1', padding: '0 24px', height: '64px', display: 'flex', alignItems: 'center', gap: '16px', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 2px 12px rgba(27,42,74,0.05)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', flexShrink: 0 }} onClick={() => router.push('/')}>
            ...
            <span className="logo-text" style={{ fontSize: '22px', color: '#1B2A4A', letterSpacing: '-0.3px', lineHeight: 1 }}>BookMart</span>
          </div>

          {/* Search */}
          <div ref={searchRef} style={{ flex: 1, position: 'relative', maxWidth: '500px' }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#FAFAF8', border: '1.5px solid #EDE9E1', borderRadius: '14px', padding: '0 14px', height: '40px', gap: '8px', transition: 'all 0.15s' }}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="6" cy="6" r="4.5" stroke="#999" strokeWidth="1.5"/><path d="M9.5 9.5L12 12" stroke="#999" strokeWidth="1.5" strokeLinecap="round"/></svg>
              <input value={search} onChange={e => { setSearch(e.target.value); setShowSuggestions(true) }}
                ...
                style={{ border: 'none', background: 'transparent', fontSize: '13.5px', outline: 'none', flex: 1, color: '#1B2A4A', fontFamily: 'DM Sans, sans-serif' }} />
              {search && <button onClick={() => { setSearch(''); setShowSuggestions(false) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: '18px', lineHeight: 1, padding: 0 }}>×</button>}
            </div>
            {showSuggestions && suggestions.length > 0 && (
              <div style={{ position: 'absolute', top: '46px', left: 0, right: 0, background: '#fff', border: '1.5px solid #EDE9E1', borderRadius: '16px', boxShadow: '0 12px 40px rgba(27,42,74,0.12)', zIndex: 100, overflow: 'hidden' }}>
                {suggestions.map((l: any) => (
                  <div key={l.id} onClick={() => { window.location.href = '/listing/' + l.id }}
                    style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid #F5F2ED' }}
                    onMouseEnter={e => (e.currentTarget.style.background = '#FAFAF8')}
                    onMouseLeave={e => (e.currentTarget.style.background = '#fff')}>
                    <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: '#F5F2ED', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0, overflow: 'hidden' }}>
                      {l.images?.[0] ? <img src={l.images[0]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : l.emoji}
                    </div>
                    <div>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#1B2A4A' }}>{l.title}</div>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '10px' }}>
            {isSignedIn ? (
              <>
                ...
                <button className="sell-btn" onClick={() => window.location.href = '/sell'} style={{ background: '#1D9E75', color: '#fff', border: 'none', borderRadius: '12px', padding: '9px 20px', fontWeight: '600', cursor: 'pointer', fontSize: '14px', fontFamily: 'Kalam, cursive', letterSpacing: '0.3px', boxShadow: '0 3px 12px rgba(29,158,117,0.25)', transition: 'all 0.15s' }}>
                  + Sell
                </button>
                <UserButton />
              </>
              ...
              <SignInButton mode="modal">
                <button className="sell-btn" style={{ background: '#1B2A4A', color: '#fff', border: 'none', borderRadius: '12px', padding: '9px 20px', fontWeight: '600', cursor: 'pointer', fontSize: '14px', fontFamily: 'Kalam, cursive', transition: 'all 0.15s' }}>
                  Sign in
                </button>
              </SignInButton>
            )}
        ...
        {/* Category pills */}
        <div style={{ background: '#fff', borderBottom: '1.5px solid #EDE9E1', padding: '0 24px', display: 'flex', gap: '4px', overflowX: 'auto' }}>
          {cats.map(cat => (
        ...
        <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
          {search && (
            <div style={{ marginBottom: '16px', fontSize: '13px', color: '#888', display: 'flex', alignItems: 'center', gap: '8px' }}>
              {filtered.length > 0
                ? <span><strong style={{ color: '#1B2A4A' }}>{filtered.length}</strong> result{filtered.length !== 1 ? 's' : ''} for "<em style={{ color: '#1D9E75' }}>{search}</em>"</span>
          ...
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '16px' }}>
              {[...Array(8)].map((_, i) => (
                <div key={i} className="skeleton" style={{ borderRadius: '18px', height: '250px' }} />
              ))}
            </div>
          ) : error ? (
            <div style={{ background: '#fff', borderRadius: '20px', padding: '48px', textAlign: 'center', border: '1.5px solid #EDE9E1' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '24px', padding: '64px 40px', textAlign: 'center', border: '1.5px solid #EDE9E1' }}>
              <div style={{ fontSize: '48px', marginBottom: '16px' }}>📭</div>
              <div className="logo-text" style={{ fontSize: '20px', color: '#1B2A4A', marginBottom: '8px' }}>Nothing here yet</div>
            ...
            <>
              {!search && <p style={{ fontSize: '11.5px', color: '#bbb', marginBottom: '16px', fontWeight: '500', letterSpacing: '0.8px', textTransform: 'uppercase' }}>{filtered.length} listings</p>}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '16px' }}>
                {filtered.map((l: any, idx: number) => (
                  <div key={l.id} className="card fade-up" onClick={() => window.location.href = '/listing/' + l.id}
                    style={{ background: '#FFFEF9', borderRadius: '18px', border: '1.5px solid #EDE9E1', overflow: 'hidden', cursor: 'pointer', boxShadow: '0 2px 8px rgba(27,42,74,0.05)', animationDelay: `${Math.min(idx * 0.04, 0.4)}s` }}>
                    <div style={{ height: '145px', background: '#F5F2ED', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '44px', position: 'relative', overflow: 'hidden' }}>
                      {l.images?.[0]
                    ...
                    <div style={{ padding: '12px 14px' }}>
                      <div style={{ fontSize: '13.5px', fontWeight: '600', marginBottom: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#1B2A4A' }}>{l.title}</div>
                      <div style={{ fontSize: '11px', color: '#bbb', marginBottom: '8px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.subtitle}</div>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                        <span className="logo-text" style={{ fontSize: '18px', color: '#1D9E75' }}>₹{l.price}</span>
                        {l.origPrice && <span style={{ fontSize: '11px', color: '#ddd', textDecoration: 'line-through' }}>₹{l.origPrice}</span>}
                      </div>
                      <div style={{ fontSize: '10.5px', color: '#bbb', marginTop: '5px' }}>📍 {l.location}</div>
                    </div>
                  </div>
                ))}
              </div>
