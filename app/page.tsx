    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
          ...
          --text-link: #888;
          --btn-outline-bg: #fff;
          --btn-outline-hover: #F5F2ED;
          --btn-outline-border-hover: #D4CFC6;
        ...
        @media (prefers-color-scheme: dark) {
          :root {
            ...
            --text-muted: #555878;
            --text-link: #6B6F88;
            --btn-outline-bg: #1A1D27;
            --btn-outline-hover: #242736;
            --btn-outline-border-hover: #3A3D5E;
        ...
        .card-hover:hover { transform: translateY(-3px); box-shadow: var(--shadow-card-hover) !important; }
        .step-card { transition: all 0.2s ease; background: var(--bg-card); border: 1.5px solid var(--border); }
        .step-card:hover { transform: translateY(-4px); }
        .cat-item { transition: all 0.15s; cursor: pointer; background: var(--bg-cat); border: 1.5px solid var(--border); }
        .cat-item:hover { background: #0F2D1F !important; border-color: #1D9E75 !important; transform: translateY(-2px); }
        @media (prefers-color-scheme: light) {
          .cat-item:hover { background: #F0FDF8 !important; }
        }
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <section style={{ padding: '72px 32px 56px', textAlign: 'center', maxWidth: '720px', margin: '0 auto' }}>
          ...
          <h1 className="kalam fade-up" style={{ fontSize: 'clamp(40px, 7vw, 64px)', fontWeight: '700', color: 'var(--text-primary)', lineHeight: 1.15, marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '40px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '56px' }}>
            {[['500+', 'listings posted'], ['₹250', 'avg. savings'], ['100%', 'free to use']].map(([num, label]) => (
              <div key={label} style={{ textAlign: 'center' }}>
                <div className="kalam" style={{ fontSize: '30px', color: 'var(--text-primary)', fontWeight: '700' }}>{num}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '14px', maxWidth: '680px', margin: '0 auto' }}>
            {sampleListings.map((l, i) => (
              <div key={i} className="card-hover" onClick={() => router.push('/marketplace')}
                style={{ background: 'var(--bg-card)', borderRadius: '18px', border: '1.5px solid var(--border)', overflow: 'hidden', cursor: 'pointer', boxShadow: 'var(--shadow-card)' }}>
        ...
        {/* Categories */}
        <section style={{ padding: '56px 32px', background: 'var(--bg-section)', borderTop: '1.5px solid var(--border)', borderBottom: '1.5px solid var(--border)' }}>
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            <h2 className="kalam" style={{ fontSize: '32px', color: 'var(--text-primary)', marginBottom: '8px' }}>Browse by category</h2>
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '36px' }}>Find exactly what you need</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px' }}>
              {categories.map(cat => (
                <div key={cat.name} className="cat-item" onClick={() => router.push('/marketplace')}
                  style={{ borderRadius: '16px', padding: '20px 12px', textAlign: 'center' }}>
        ...
        {/* How it works */}
        <section style={{ padding: '64px 32px', textAlign: 'center', background: 'var(--bg)' }}>
          <div style={{ maxWidth: '760px', margin: '0 auto' }}>
            <h2 className="kalam" style={{ fontSize: '32px', color: 'var(--text-primary)', marginBottom: '8px' }}>How it works</h2>
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '40px' }}>Three steps. No fees. No friction.</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }}>
              {[
                ...
                <div key={s.step} className="step-card" style={{ borderRadius: '20px', padding: '28px 20px', textAlign: 'center', boxShadow: 'var(--shadow-card)' }}>
                  <div style={{ fontSize: '36px', marginBottom: '14px' }}>{s.icon}</div>
                  <div className="kalam" style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px', letterSpacing: '1px' }}>STEP {s.step}</div>
                  <div style={{ fontSize: '15px', fontWeight: '600', color: 'var(--text-primary)', marginBottom: '8px' }}>{s.title}</div>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>{s.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* CTA — always dark, looks great in both modes */}
        ...
        {/* Footer */}
        <footer style={{ borderTop: '1.5px solid var(--border)', padding: '24px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', background: 'var(--bg)' }}>
